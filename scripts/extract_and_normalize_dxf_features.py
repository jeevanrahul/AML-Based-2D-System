...
if not all_shapes_data:
    ...
    save_extracted_folder = r"C:\Users\welcome\Desktop\New folder\AML-Based-2D-System\data\processed\features"
    save_normalized_folder = r"C:\Users\welcome\Desktop\New folder\AML-Based-2D-System\data\processed\normalized_features"

    os.makedirs(save_extracted_folder, exist_ok=True)
    os.makedirs(save_normalized_folder, exist_ok=True)

    # Get shape name from DXF filename (without extension)
    shapename = os.path.splitext(os.path.basename(filename))[0]

    # Prepare file paths
    extracted_file_path = os.path.join(save_extracted_folder, f"features_{shapename}.csv")
    normalized_file_path = os.path.join(save_normalized_folder, f"normalized_{shapename}.csv")

    # Save extracted features
    df = pd.DataFrame(all_shapes_data)
    df.to_csv(extracted_file_path, index=False)
    print(f"Extracted features saved successfully at: {extracted_file_path}")

    # Extract numeric columns only for normalization
    ...
    if numeric_df.empty:
        ...
        print(f"Normalized features saved successfully at: {normalized_file_path}")
